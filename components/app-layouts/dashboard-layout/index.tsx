...
import { Button, Layout, Menu, Drawer, message } from "antd";
...
function getItem(
  ...
  // If there are children (submenu), don't make it clickable
  // Otherwise, wrap in Link if href is provided
  return {
    key,
    icon,
    children,
    label: children ? label : href ? <Link href={href}>{label}</Link> : label,
  } as MenuItem;
}
  ...
  getItem(
    ...
    [
      getItem(
        ...
        undefined,
        "/dashboard/settings?type=payment_method"
      ),
      getItem(
        "Update Password",
        "update-password",
        ...
        undefined,
        "/dashboard/settings?type=update_password"
      ),
    ]
    // No href - parent menu with children should not be clickable
  ),
