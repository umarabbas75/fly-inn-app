...
export const ImagesForm: React.FC<ImagesFormProps> = ({ maxNumber = 70 }) => {
  ...
  console.log({ images, imgdescription, deletedImages });
    ...
    if (images && images.length > 0) {
      ...
      const processedInitialImages = sortedImages.map((img: ImageItem) => {
        ...
        if (img.data_url) {
          ...
          // Backend format: { id, url, image, description, file_name, sort_order }
          // Use url if available (full URL), otherwise construct from image path
          const fullImageUrl =
            img.url ||
          ...
          return {
            ...
            file: undefined, // No file object for existing images unless re-uploaded
            description: img.description || "", // Backend provides description
            sort_order: img.order ?? img.sort_order ?? 0, // Map 'order' to 'sort_order'
            image: img.image, // Keep original image path for reference
            url: img.url, // Keep original URL for reference
            file_name: img.file_name, // Keep file_name for reference
          } as ImageItem;
        }
      });
      console.log(
  ...
  // Check image resolution (minimum 1024×683 pixels)
  const checkImageResolution = (file: File): Promise<boolean> => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        const isValid = img.width >= 1024 && img.height >= 683;
        resolve(isValid);
      };

      img.onerror = () => {
        URL.revokeObjectURL(objectUrl);
        resolve(false); // If we can't load the image, reject it
      };
      ...
      for (const imageItem of newImageList) {
        if (imageItem.file) {
          ...
          if (!isValidResolution) {
            message.error(
              `Image "${fileToProcess.name}" does not meet minimum resolution requirement (1024×683 pixels)`
            );
            continue; // Skip this image
          }
    ...
    <DndProvider backend={HTML5Backend}>
      ...
      >
        <h2 className="flex items-center text-lg sm:text-xl font-bold text-gray-800 mb-3 sm:mb-4 md:mb-6">
          ...
          <FieldLabel label="Property Images" required={true} />
        </h2>
        ...
        <div className="mb-4 sm:mb-5 md:mb-6">
          <p className="text-xs sm:text-sm text-gray-600 mb-3 sm:mb-4">
            ...
            can drag and drop images to reorder them.
          </p>
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <ul className="text-[10px] sm:text-xs text-primary/70 space-y-0.5 sm:space-y-1">
                  ...
                  <li>
                    • Use photos with a minimum resolution of 1024×683 pixels
                  </li>
                </ul>
              </div>
            </div>
